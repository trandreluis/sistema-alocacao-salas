...
public class ExibicaoEvento implements Exibicao {
	...
	/**
	 * Metodo responsavel por interagir com o usuario a fim de obter uma caracteristica de um Evento
	 */
	...
	menu();
		...
		if(id.length() != 5 || !str.equals("EV")) {
			System.out.println("Operacao abortada: O ID informado nao e valido.");
			menu();
		}
		
		if(!opEvento.verificarExistencia(id)) {
			System.out.println("Operacao abortada: O ID informado nao pertence a nenhum evento cadastrado.");
			menu();
		}
	...
	public void remover() {
		...
		System.out.println("Deseja listar os eventos? 1 - Listar, 2 - Remover sem listar.");
		Scanner sc5 = new Scanner(System.in);
		
		String opcaoListagem = sc5.nextLine();
		
		if(!opcaoListagem.equals("1") && !opcaoListagem.equals("2")) {
			System.out.println("Operacao abortada: Escolha invalida, esta nao e uma das opcoes.");
			menu();
