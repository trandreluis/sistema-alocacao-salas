...
public class EventoDao extends GenericDao {
	...
	public void salvar(Evento evento) {
		...
		String sql = "INSERT INTO EVENTOS (id_evento, nome, inicio, fim, area, contato, repeticoes)"
				+ "VALUES (?,?,?,?,?,?,?)";

		super.salvar(sql, evento.getIdEvento(), evento.getNome(),
				evento.getInicio(), evento.getFim(), evento.getArea(),
				evento.getContato(), evento.getRepeticoes());

	}
	
	/**
	 * Metodo que realiza uma busca na tabela Eventos pelo ID
	 * @param idEvento ID de um Evento (ex.: EV-12)
	 ...
	 * @param atributo Atributo do evento que sera retornado
	 ...
	 * @param idEvento ID do Evento a ser testado
	 ...
	 * @param idEvento ID do Evento que sera apagado
	 */
