...
public class OuvinteAdicionarSala implements ActionListener {
	...
	public void verificaCadastroEDecide() {
		...
		else if (capacidade == null || capacidade.trim().equalsIgnoreCase("")) {
			JOptionPane.showMessageDialog(null, "Informe a Capacidade!");
			janelaAdicionar.botaoCadastrarSala.setEnabled(true);
		}

		else if (finalidade == null || finalidade.trim().equalsIgnoreCase("")) {
		...
		else if (comecaOuTerminaComEspaco(apelido)) {
			if (comecaOuTerminaComEspaco(apelido)) {
				JOptionPane.showMessageDialog(null,
						"O apelido nao deve comcecar ou terminar com espacos!");
				janelaAdicionar.fieldApelido.setText("");
				janelaAdicionar.botaoCadastrarSala.setEnabled(true);
			}
		}
				...
				if (capacidadeInt <= 0) {
					JOptionPane
							.showMessageDialog(null,
									"A capacidade nao pode ser menor ou igual a 0 (zero)!");
					janelaAdicionar.botaoCadastrarSala.setEnabled(true);
				}

				else if (finalidade
						.equalsIgnoreCase("-- selecione a finalidade da sala --")
						&& tipo.equalsIgnoreCase("-- selecione o tipo da sala --")) {
					JOptionPane.showMessageDialog(null,
							"Informe uma Finalidade e um Tipo!");
					janelaAdicionar.botaoCadastrarSala.setEnabled(true);
				}
						...
						.equalsIgnoreCase("-- selecione a finalidade da sala --")) {
					JOptionPane.showMessageDialog(null,
							"Informe uma Finalidade!");
					janelaAdicionar.botaoCadastrarSala.setEnabled(true);
				}

				else if (tipo
						.equalsIgnoreCase("-- selecione o tipo da sala --")) {
	...
	public void cadastrarSala(String id, int capacidade, String finalidade,
		...
		try {
			janelaPrincipal.fachada.adicionarSala(id, capacidade, finalidade, tipo,
					apelido, aberta);
